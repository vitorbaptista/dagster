...
Imagine a machine learning job with three steps. The first step, training the model, is the most time and resource intensive. Then, we test the model, and build analyses on the results. Suppose that the job fails with the step that is testing the model. After fixing the root cause, we want to re-run our job. However, it would take much more time to create a new run of the job as we would have to repeat the first step. It would be more economical to start again from the second step, reusing the previous run's execution result for the first step.
...
Although very simple, there are inputs and outputs passed between steps. With an IO manager, re-execution is able to handle inputs and outputs stored from the initial run.

To initiate a re-execution from an existing run, navigate to the run on Dagit and you can find the re-execution option on the top right of the interface.
...
- **All Steps**: Re-execute the job from scratch. This option is most relevant if you would like to associate runs together when testing jobs end-to-end.
- **Selected Step(s)**: Re-execute the selected step(s). Steps can be selected regardless of their step status. This option is most relevant if your job is large, and you know exactly which steps to execute. This can be done by clicking on the boxes in the gantt chart view.
- **From Selected**: Re-execute the job downstream from the selected steps. This option is most relevant if a particular step fails, and your intent is to run all downstream steps regardless of step status. You are likely developing a single step, and want to make sure downstream steps work as expected.
- **From Failure**: Re-execute the job, skipping steps completed successfully. This option is only enabled when the run has failed. You have likely fixed the failed step, and want to re-run the step and all downstream dependencies. Dagster will figure out the dependencies for you!
...
If the run succeeded but the underlying code changed, running specific steps to test the differences would be more relevant.

### Selecting Steps

Within Dagit, a single or multiple steps may be selected simply by clicking them with the mouse. Alternatively, you can use the subset selector and specify your desired step names to re-run.
