...
This guide explains how you can run a [dbt](https://docs.getdbt.com/docs/introduction) project as part of a Dagster graph.
...
Dagster orchestrates dbt alongside _other technologies_, so you can combine dbt with Spark, Python, etc. in a single graph. Dagster also provides built-in operational and data observability capabilities, like storing dbt run results longitudinally and sending alerts when a dbt run fails.
...
One common way to use this integration is to have the resource run all of the models in a dbt project. To do this, just configure the resource so it knows where your dbt project is, and fire off a `dbt.run()` command!  Notice that, to supply the resource to the graph, we turn the graph into a job.
...
#### Invoking multiple dbt commands in the same graph

Sometimes, you'll want to run multiple different dbt commands in the same graph. The `dbt_cli_resource` makes this convenient, as you only need to configure your dbt resource once, and all of that configuration will already be set for any ops that are using this resource.
...
## Using a dbt RPC server in a Dagster graph

Alongside the `dbt_rpc_resource` (which functions similarly to the `dbt_cli_resource`), `dagster-dbt` provides ops for running commands through the [dbt RPC server](https://docs.getdbt.com/reference/commands/rpc). By convention, these ops are named `dagster_dbt.dbt_rpc_*`.
...
Your dbt RPC server can be running locally or remotely. To use the dbt RPC ops in your Dagster graph, you will need to create a resource for your dbt RPC server.
...
The code snippet above shows a Dagster graph with a single op `dbt_rpc_run`. The op `dbt_rpc_run` has a required resource key `"dbt_rpc"`. So, any job that uses `dbt_rpc_run` will need a resource under the key `"dbt_rpc"`.
...
## Use dbt Cloud in a Dagster graph
